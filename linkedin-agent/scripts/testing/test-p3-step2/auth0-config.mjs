...
console.log('  File: test-p3-step2/auth0-config.mjs');
...
test('3.2.1.1', ' Clearing AUTH0_DOMAIN, AUTH0_CLIENT_ID, AUTH0_CLIENT_SECRET', () => {
...
test('3.2.1.2', ' Setting AUTH0_DOMAIN=test.auth0.com (1 of 3 credentials)', () => {
...
test('3.2.1.3', ' Adding AUTH0_CLIENT_ID=cid (2 of 3 credentials)', () => {
...
test('3.2.1.4', ' Adding AUTH0_CLIENT_SECRET=secret (3 of 3 credentials)', () => {
...
test('3.2.1.5', ' Provider rejects empty domain', () => {
...
test('3.2.1.6', ' Setting AUTH0_DOMAIN to whitespace-only', () => {
...
test('3.2.2.1', ' Setting AUTH0_DOMAIN=test.auth0.com (clean, no protocol)', () => {
...
test('3.2.2.2', ' Setting AUTH0_DOMAIN=https://test.auth0.com', () => {
...
test('3.2.2.3', ' Setting AUTH0_DOMAIN=http://test.auth0.com/', () => {
...
test('3.2.2.4', ' Setting AUTH0_DOMAIN=test.auth0.com/', () => {
...
test('3.2.2.5', ' Issuer URL correct', () => {
  process.env.AUTH0_DOMAIN = 'https://test.auth0.com/';
...
test('3.2.2.6', ' JWKS URI is where signing keys are published', () => {
  var cfg = auth0._getConfig();
...
test('3.2.2.7', ' Token URL is called during code→token exchange', () => {
  var cfg = auth0._getConfig();
...
test('3.2.2.8', ' UserInfo URL retrieves the authenticated user profile', () => {
  var cfg = auth0._getConfig();
...
await testAsync('3.2.6.1', '  init() with AUTH0_DOMAIN missing', async () => {
...
await testAsync('3.2.6.2', ' Setting AUTH0_DOMAIN=nodots (no dots)', async () => {
...
await testAsync('3.2.6.3', ' Setting AUTH0_DOMAIN with spaces', async () => {
...
await testAsync('3.2.6.4', '  init() without AUTH0_CLIENT_SECRET', async () => {
...
await testAsync('3.2.6.5', '  init() with all three valid credentials', async () => {
